...
async function getRpi5In83BV2(orientation?: Orientation, colorMode?: ColorMode): Promise<DisplayDevice> {
    try {
        const { Rpi5In83BV2 } = await import('@epaperjs/rpi-5in83b-v2');
        return new Rpi5In83BV2(orientation, colorMode);
    } catch (e) {
        throw new Error('Failed to import @epaperjs/rpi-5in83b-v2, make sure it is installed');
    }
}